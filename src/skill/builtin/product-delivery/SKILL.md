...
description: 产品交付工作流。用于开发类需求，当用户要求你帮他实现某个代码开发类的功能/应用时触发；必须按五步统一流程执行：项目初始化、需求分析、架构设计、编码实现、基础验证。
---
...
5. 基础验证：按轻量预算执行可用验证，如文件入口检查、语法检查等。
...
基础验证只做低成本交付自检，不追求完整测试矩阵。先检查文件结构、入口文件、配置和静态资源引用等零依赖项目，不要为了验证临时安装依赖、全局初始化环境或长时间等待外部工具。
